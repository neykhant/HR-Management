        ...
        <form action="{{ route('attendance.update', $attendance->id) }}" method="POST" id="edit-form">
            ...
            <div class="form-group">
                ...
                <select name="user_id" class="form-control select-ninja">
                    ...
                    <option value="{{ $employee->id }}" @if(old('user_id', $attendance->user_id ) == $employee->id ) selected @endif> {{ $employee->id }} ({{ $employee->name }}) </option>
                    @endforeach
                </select>
            </div>
...
{!! JsValidator::formRequest('App\Http\Requests\UpdateAttendanceRequest', '#edit-form') !!}
